...
def process_url(url: str) -> dict:
    """Send URL to the crawler API for processing"""
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"API Error: {str(e)}. Make sure the API server is running on port 8000."
        }
    except Exception as e:
...
with tab2:
    ...
    # Add URL Processing section at the top
    st.subheader("Process URL")
    url_input = st.text_input("Enter URL to process", placeholder="https://example.com")
    ...
    # Existing document management code
